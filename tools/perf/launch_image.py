...
def collect_bootspeed_data(results_dir, cloud, image_id, user_data):
    ...
    for service in (
        "cloud-init.target",  # measures time to completion
...
def launch_instances(
    ...
    with PLATFORM_FROM_STR[platform](tag="examples") as cloud:
        image = cloud.daily_image(release=image) if image in ["bionic", "focal", "jammy", "noble", "oracular"] else image
...
def get_parser():
    ...
    parser.add_argument(
        "--image",
        dest="image",
        help=("Image or series name to build from, defaults to daily image"),
    )
    parser.add_argument(
        "--series",
        dest="image",
...
if __name__ == "__main__":
    ...
    print(f"Results saved to: {temp_dir.name}/orig.json")
