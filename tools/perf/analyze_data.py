...
def process_critical_chain_data(boot_sample: dict):
    # TODO
    # the following should capture unit name, unit time, and optionally unit "blocking time"
    # ([a-z\.\-]+) @([\d.]+[smh]?)(?: \+([\d.]+[smh]?))?
    for match in re.findall(
        r"([a-z\.\-]+) @([\d.]+[smh]?)(?: \+([\d.]+[smh]?))?",
        boot_sample["criticalchain_cloud_init_target"],
    ):
        boot_sample["cloud_init_complete"] = float(match[1][:-1])
        break
...
def analyze_data(data_file: Path):
    ...
    file = Path(data_file)
    orig_data = json.loads(file.read_text())
    orig_avg = update_averages(orig_data, file.stem, "orig")
    print(
        HEADER.format(
...
def get_parser():
    ...
    parser.add_argument(
        "--data-file",
        dest="data_file",
        help="Data file in which to store perf value dicts.",
    )
    return parser
...
if __name__ == "__main__":
    ...
    analyze_data(data_file=args.data_file)
